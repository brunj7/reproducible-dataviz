...
This example aims to showcase a use of markdown to create documentation. In this synthesis example, we showcase a subset of the data processing workflow developed by the NCO synthesis working group [Stream Elemental Cycling](https://lternet.edu/working-groups/global-patterns-stream-energy-nutrient-cycling/). 
...
# Create the map 
kml <- readr::read_file("./data/streams.kml")
...
```{r read_data, eval=FALSE, message=FALSE}
...
RioMameyesPuenteRoto_data$Gage_Ht <- ud.convert(RioMameyesPuenteRoto_data$Gage_Ht,
                                                RioMameyesPuenteRoto_attrmeta$unit[RioMameyesPuenteRoto_attrmeta$attributeLabel == "Gage height"], "meter")
...
## Doing some exploratory data visualization

```{r}
library(ggplot2)

ggplot(all_sites_luq) + 
    geom_line(aes(Sample_Date, Cond, color=Sample_ID))
```

**=> not all the site have the same time-series length! 2001-2013 seems to be a good period of overlap.**